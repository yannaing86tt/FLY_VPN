...
public class Tunnel
{
	...
	public synchronized boolean startRouting(TunnelVpnSettings settings) throws java.lang.Exception
	{
		return startVpn(settings.mDnsForward, settings.mDnsResolver, settings.mExcludeIps,
			settings.mEnableFilterApps, settings.mFilterBypassMode, settings.mFilterApps, settings.mTetheringSubnet, settings.bypass);
	}
	...
	private boolean startVpn(boolean forwardDns, String[] dnsResolver, String[] excludeIps,
			boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering, boolean useBypass) throws java.lang.Exception
	{
...
//					mHostService.onDiagnosticMessage("Route rejeitada: " + route + " " + ia.getLocalizedMessage());
			...
			if (Build.VERSION.SDK_INT >= 21) {
                if (useBypass) {
                    mHostService.onDiagnosticMessage("<strong>SlowDNS Bypass OK</strong>");
			...
			tunFd = builder
					...
					.setConfigureIntent(MainService.getGraphPendingIntent(mHostService.getContext()))
					.establish();
	...
	private void stopVpn()
	{
		...
		if (tunFd != null)
		{
			try
			{
				mHostService.onDiagnosticMessage("VPN service stopped");
				tunFd.close();
			}
			catch (IOException e) {}
		}
